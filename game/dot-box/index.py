...
try:
    ...
    HAS_PYGAME_TEXTINPUT = True
except ImportError:
    HAS_PYGAME_TEXTINPUT = False
import random
import math
import os
import json
...
# --- Unicode TextInput hỗ trợ tiếng Việt ---
if HAS_PYGAME_TEXTINPUT:
    class TextInput:
        ...
        def handle_event(self, event):
            ...
            if self.active:
                consumed = self.textinput.update([event])
...
class GameUI:
    def __init__(self):
        ...
        # Fonts: ưu tiên các font hệ thống hỗ trợ Unicode/tiếng Việt
        unicode_fonts = [
            "Segoe UI", "Tahoma", "DejaVu Sans", "Arial Unicode MS", "Arial", "Liberation Sans"
        ]

        def get_unicode_font(size, bold=False):
            for fname in unicode_fonts:
                try:
        ...
        self.font_title = get_unicode_font(max(42, self.width // 24), bold=True)
        self.font_big = get_unicode_font(max(30, self.width // 40), bold=True)
        self.font_mid = get_unicode_font(max(22, self.width // 60), bold=True)
        self.font_small = get_unicode_font(max(18, self.width // 80))
        self.font_tiny = get_unicode_font(max(16, self.width // 90))
    ...
    def load_settings(self):
        ...
        default_music = self.get_default_music_key()
        defaults = {
            "language": "en",
            "volume": 70,
            "bg_music": default_music,
        ...
        self.selected_music_key = data.get("bg_music", default_music)
        if self.selected_music_key not in self.music_options:
            self.selected_music_key = default_music
                ...
                json.dump(
                    {
                        ...
                        "bg_music": self.selected_music_key,
    ...
    def load_music_options(self):
        self.music_options = {}
        music_dir = os.path.join(os.getcwd(), "music")
        if not os.path.isdir(music_dir):
            return

        for music_key in ("mck", "nevada"):
            for ext in (".mp3", ".ogg", ".wav"):
                path = os.path.join(music_dir, f"{music_key}{ext}")
                if os.path.exists(path):
                    self.music_options[music_key] = path
                    break

    def get_default_music_key(self):
        if "nevada" in self.music_options:
            return "nevada"
        if "mck" in self.music_options:
            return "mck"
        for key in self.music_options:
            return key
        return ""

    def get_music_label(self, music_key, lang=None):
        use_lang = lang or self.language
        if not music_key:
            return self.tr_with_lang("music_none", use_lang)
        return self.tr_with_lang(f"music_{music_key}", use_lang)

    def set_background_music(self, music_key):
        if music_key not in self.music_options:
            return
        self.selected_music_key = music_key
        self.apply_music(force_reload=True)
        self.save_settings()
    ...
    def resolve_music_path(self):
        selected = self.music_options.get(self.selected_music_key, "")
    ...
    def change_state(self, new_state):
        ...
        if new_state == "MENU":
            ...
            self.buttons = [
                Button(rects[0], self.tr("player_vs_ai"), self.goto_names_ai, font=self.font_mid),
            ...
            input_x = self.frame_names.x + 58
            avatar_center_x = self.frame_names.right - 140
            avatar_control_w = 52
            avatar_control_h = 40
            avatar_control_gap_x = 74

            self.name_input_1 = TextInput(
                pygame.Rect(input_x, self.frame_names.y + 220, input_w, input_h),
            ...
            self.name_input_2 = TextInput(
                pygame.Rect(input_x, self.frame_names.y + 320, input_w, input_h),
            ...
            avatar_row1_y = self.name_input_1.rect.centery
            avatar_row2_y = self.name_input_2.rect.centery
            ...
            self.buttons = [
                ...
                Button(
                    pygame.Rect(
                        avatar_center_x - avatar_control_gap_x - avatar_control_w // 2,
                        avatar_row1_y - avatar_control_h // 2,
                        avatar_control_w,
                        avatar_control_h,
                    ),
                    "<",
                ...
                Button(
                    pygame.Rect(
                        avatar_center_x - avatar_control_gap_x - avatar_control_w // 2,
                        avatar_row2_y - avatar_control_h // 2,
                        avatar_control_w,
                        avatar_control_h,
                    ),
                    "<",
            ...
            ai_row_rect = pygame.Rect(input_x, self.frame_names.y + 320, input_w, input_h)
            avatar_row1_y = self.name_input_1.rect.centery
            avatar_row2_y = ai_row_rect.centery

            self.buttons = [
                Button(pygame.Rect(cx - 175, self.frame_names.bottom - 130, 350, 58),
                       self.tr("continue"), self.goto_difficulty_from_ai_names, font=self.font_mid),
                Button(pygame.Rect(cx - 80, self.frame_names.bottom - 58, 160, 48),
                       self.tr("back"), self.go_back, font=self.font_small),
                Button(
                    pygame.Rect(
            ...
            music_mck_available = "mck" in self.music_options
            music_nevada_available = "nevada" in self.music_options
            mck_selected = self.selected_music_key == "mck"
            nevada_selected = self.selected_music_key == "nevada"
            ...
            self.buttons = [
                ...
                Button(
                    pygame.Rect(control_x, row3_y - 8, 142, 42),
                    self.tr_with_lang("music_mck", self.pending_language),
                    (lambda: self.set_background_music("mck")) if music_mck_available else None,
                    base_color=(198, 236, 214) if mck_selected else THEME["panel2"],
                    hover_color=(140, 214, 177) if mck_selected else THEME["accent"],
                    font=self.font_mid,
                ),
                Button(
                    pygame.Rect(control_x + 158, row3_y - 8, 142, 42),
                    self.tr_with_lang("music_nevada", self.pending_language),
                    (lambda: self.set_background_music("nevada")) if music_nevada_available else None,
                    base_color=(198, 236, 214) if nevada_selected else THEME["panel2"],
                    hover_color=(140, 214, 177) if nevada_selected else THEME["accent"],
                    font=self.font_mid,
                ),
                Button(
    ...
    def goto_names_ai(self):
        self.play_mode = "AI"
        self.change_state("NAMES_AI")
    ...
    def goto_difficulty_from_ai_names(self):
        n1 = self.name_input_1.text.strip() if self.name_input_1 else ""
        self.player1_name = n1 if n1 else "PLAYER 1"
        self.change_state("DIFFICULTY")
    ...
    def draw_names_menu(self):
        ...
        if self.name_input_1:
            draw_text_center(
                ...
                self.font_small,
                THEME["player"],
                (self.name_input_1.rect.centerx, self.name_input_1.rect.y - 24),
            )
        ...
        if self.name_input_2:
            draw_text_center(
                ...
                self.font_small,
                THEME["p2"],
                (self.name_input_2.rect.centerx, self.name_input_2.rect.y - 24),
            )
            self.name_input_2.draw(self.screen)

        avatar_x = fr.centerx
        avatar_row1_y = fr.y + 420
        avatar_row2_y = fr.y + 500
        if len(self.buttons) >= 6:
            avatar_x = (self.buttons[2].rect.centerx + self.buttons[3].rect.centerx) // 2
            avatar_row1_y = self.buttons[2].rect.centery
            avatar_row2_y = self.buttons[4].rect.centery

        draw_text_center(self.screen, self.tr("avatar_p1"), self.font_tiny, THEME["text"], (avatar_x, avatar_row1_y - 44))
        draw_text_center(self.screen, self.tr("avatar_p2"), self.font_tiny, THEME["text"], (avatar_x, avatar_row2_y - 44))

        has_icons = len(self.avatar_options) > 0
        if has_icons:
            self.draw_player_avatar((avatar_x, avatar_row1_y), player_index=1)
            self.draw_player_avatar((avatar_x, avatar_row2_y), player_index=2)
        else:
            draw_text_center(
                self.screen,
                self.tr("no_icons"),
                self.font_tiny,
                THEME["danger"],
                (avatar_x, (avatar_row1_y + avatar_row2_y) // 2),
            )

        if not has_icons and len(self.buttons) >= 6:
            for i in [2, 3, 4, 5]:
    ...
    def draw_names_ai_menu(self):
        ...
        ai_label_x = self.name_input_1.rect.centerx if self.name_input_1 else fr.centerx
        draw_text_center(self.screen, "AI", self.font_small, THEME["ai"], (ai_label_x, avatar_row2_y - 24))
    ...
    def draw_settings_menu(self):
        ...
        trp = lambda key, **kwargs: self.tr_with_lang(key, preview_lang, **kwargs)
        draw_round_rect(self.screen, fr, THEME["panel"], 30)
        draw_round_rect_outline(self.screen, fr, (255, 255, 255), 2, 30)
        ...
        draw_text_left(self.screen, f"{trp('music').upper()}:", self.font_mid, THEME["text"], (label_x, row3_y))
        ...
        current_music_label = self.get_music_label(self.selected_music_key, preview_lang)
        draw_text_left(
            self.screen,
            trp("current", value=current_music_label),
            self.font_tiny,
            THEME["accent"],
            (fr.x + 360, row3_y + 48),
        )
    ...
    def draw(self):
        ...
        elif self.state == "NAMES_AI":
            self.draw_names_ai_menu()
        ...
        while self.running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if self.state == "GAME":
                        self.go_back_from_game()
                    elif self.state in ["SIZE", "DIFFICULTY", "GAMEOVER", "NAMES", "NAMES_AI", "SETTINGS"]:
                        self.go_back()
                    else:
                        self.running = False
                ...
                if self.state == "NAMES_AI":
                    if self.name_input_1:
                        res1 = self.name_input_1.handle_event(event)
                        if res1 == "submit":
                            self.goto_difficulty_from_ai_names()
